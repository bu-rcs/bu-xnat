...
* Kyle decided to take a stab at creating a custom docker container to complete the entirety of the DICOM --> MRIQC pipeline within a single container. The code for this container is contained within [this github repository]() and the build docker container is [publicly available here on dockerhub](). The solution Kyle wrote builds upon the MRIQC docker container and adds in the package dcm2bids -- which appears to be a modern, 2-3 line solution for converting DICOM formatted scans to BIDS formatted NIFTI scans (see the Github repo README for more information.
* In order for Kyle's solution to work, you need to upload a bids2dcm `bids_config.json` file to the XNAT server using the `/data/config` API. See the Github repo and the dcm2bids documentation for more information.
