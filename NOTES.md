### November 8th, 2024
* Kyle figured out a fix to the problematic curl commands detailed in the previous update to this document. See [Kyle's XNAT Google Group thread](https://groups.google.com/g/xnat_discussion/c/wODhMiQfwRI). The TL;DR -- the curl command required using the `--file-upload` flag instead of the `-d` flag to upload the bidsmap JSON file to the XNAT server.
* After speaking with Stephanie last week, Kyle decided to move away from John Flavin's solution to the "how to run MRIQC within XNAT's container service" problem. John Flavin's approach is ~6 years old, involves chaining together 2-3 different XNAT enabled docker containers, and involves writing NIFTI versions of the scans back to the XNAT server, effectively doubling the amount of storage space required. Stephanie requested that we try and avoid writing the intermediate NIFITI files back to the XNAT server if possible.
* Kyle decided to take a stab at creating a custom docker container to complete the entirety of the DICOM --> MRIQC pipeline within a single container. The code for this container is contained within [this github repository](https://github.com/bu-rcs/dcm2mriqc) and the build docker container is [publicly available here on dockerhub](https://hub.docker.com/r/kkurkela92/dcm2mriqc). The solution Kyle wrote builds upon the MRIQC docker container and adds in the package dcm2bids -- which appears to be a modern, 2-3 line solution for converting DICOM formatted scans to BIDS formatted NIFTI scans (see the Github repo README for more information.
* In order for Kyle's solution to work, you need to upload a bids2dcm `bids_config.json` file to the XNAT server using the `/data/config` API. See the [Github repo](https://github.com/bu-rcs/dcm2mriqc) and the [dcm2bids documentation](https://unfmontreal.github.io/Dcm2Bids/3.2.0/) for more information.
...
<body>
<h3>You must specify either the status querystring parameter or a configuration object in the request body.</h3><p>You can get technical details <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html#sec10.4.1">here</a>.<br>
